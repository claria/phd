...
The goal of this measurement is the comparison of the dijet production cross section with
higher-order calculations at particle level. Due to a finite detecter acceptance and resolution
the measured spectrum is smeared causing differences between the true dijet \ptavg distribution
and the measured distributiong. The aim of the unfolding procedure is the removal of these effects
by applying an unfolding algorithm.

Following several other QCD analyses, the iterative d'Agostini algorithm~\cite{D'Agostini:1994zf} was used which is
implemented in the RooUnfold~\cite{Adye:2011gm} package. The number of
steps in the iterative procedure is a regularization parameter of the algorithm. A high number
of iterations leads to an increasing uncertainty and larger bin-by-bin fluctuations. The regularization
parameter was studied using the MG+P6 MC sample and the best result is achieved using four iterations.

The response matrix used in the unfolding procedure was studied using two methods. The first one
is using the Monte-Carlo samples processed through the detector simulation and event-by-event the true and the measured entries are
filled into the response matrix taking into account the missed and fake hits.. While
this is the most straight-forward and easy to implement method, it has several drawbacks. All used MC samples are only
leading-order and the shape of the measured distributions in not well described in all cases. Additionally
the number of events in these samples is limited leading to large statistical fluctuations in less populated
phase space regions in the response matrix. 

Therefore the second method based on the forward smearing of a fitted NLO
spectrum is used. For each of the measurement bins, the resolution was measured
as shown in Section \ref{sec:resolution}. Using a Toy MC method a flat \ptavg
spectrum was generated, which is weighted according to the NLO distribution
obtained using fastNLO evaluated with the CT14-NLO PDF set and corrected for
non-perturbative effects. The distribution of this generated sample resembles
the fastNLO cross section and is used as the true \ptavg spectrum. This cross
section is then smeared using gaussian distributed values generated from the
resolution function obtained before. This allows to fill the response matrix with 
a very high number of events avoiding any statistical fluctuations. A crosscheck has been 
done comparing the results of the unfolding using the MC sample and the smeared NLO spectrum
and the agreement was found to be good within the statistical uncertainties of
the MC samples.

It was also checked that the bin-by-bin migrations between neigbored \ystar and
\yboost bins are small. This allows to perform a one-dimensional unfolding
instead of a three-dimensional unfolding.

The response matrices from the NLO smearing method which are used in the
unfolding procedure are shown in Figure~\ref{fig:res_matrix}. The matrices in
the figure are normalized to number of gen events in each row to improve the
readibility. The response matrices are diagonal with small off-diagonal
migrations between close-by \ptavg bins.

The statiscal uncertainties is propagated through the unfolding using a toy MC procedure
in which the measured histogram is smeared according to its statistical uncertainty and the
unfolding procedure is performed for each of these smeared spectrum as it is implemented in 
the RooUnfold package. One million generated toy spectra were used to propagate the statistical
uncertianty. Figure~\ref{fig:statunc_relative} shows the statistical uncertainty before and after
the unfolding. The uncertainty slightly increases during the unfolding process as it is expected.
