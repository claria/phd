...
corrections for dijet calculations. When these corrections will become publicly
...
dijets are directly related to the fractional proton momenta. So it appears natural,
...
second jet. However, this introduces correlations between phase space regions far
...
The phase space cuts on the transverse momenta and rapidities of the jets are
motivated by detector acceptance, trigger efficiencies and furthermore ensure a phase
...
rapidity, both jets must be boosted into the forward region to the same side, see
...
The NLO predictions of the triple-differential dijet cross section are
...
reflects the energy scale of the measurement.
...
For the final comparisons between data and NLO predictions and for the PDF
studies presented later in this thesis, the scale choice $\mu =
p_{\mathrm{T,max}} e^{0.3 \ystar}$ is used, since it exhibits the smallest scale
uncertainties.
...
\begin{figure}[htp]
    ...
    \caption[\NLOJETPP prediction of triple-differential dijet cross section]{
        ...
        \SI{10}{\percent}. In
        almost all cases the differences between the calculations are however covered by
...
rapidity separation of $2 \leq \ystar < 3$ are smaller than unity for the
\ptavg scale choice, while it is larger than unity for the scale choices including
...
\begin{figure}[htp]
    ...
    \caption[NLO $k$-factors of \NLOJETPP calculation]{The $k$-factors between the NLO and LO calculation
...
correction is then applied as a bin-by-bin correction factor to the parton-level NLO cross
section.
...
larger differences, an uncertainty is assigned to the correction
factor. The envelope which covers all differences is taken as uncertainty,
while the mean of the envelope is used as correction
factor.
...
in the phase space regions containing boosted jets. 
...
already presented together with the obtained correction factors in
...
neglected higher-order contributions are covered by a systematic uncertainty.
...
uncertainty sources of the data included in the PDF fit.
...
Fig.~\ref{fig:pdf_uncertainties} shows the relative PDF uncertainties for the
three studied global PDF sets. The PDF uncertainty in the bins with small
...
region of the proton PDFs is accessed which is well known already. The PDF
...
bin with largest values of \yboost.  Especially in
...
\begin{figure}[htp]
    ...
    \caption[PDF uncertainties]{The relative PDF uncertainties are shown for the three PDF sets
        NNDFP 3.0, CT14, and MMHT 2014. The uncertainty represents a
        \SI{68}{\percent} confidence
    interval. The PDF uncertainty is sizable especially in the boosted region.}
...
\begin{figure}[htp]
    ...
    \caption[Overview of theoretical uncertainties]{Overview of theoretical
        uncertainties of the NLO predictions.
    The scale uncertainty is the dominant uncertainty in the low-\pt region. At
    high-\pt and especially in the boosted region, the PDFs become the dominant
