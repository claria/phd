...
runs provides the base data set as it covers most of the kinematic phase space.
...
the PDFs are studied by performing a combined fit of the HERA DIS data and the
...
cross section has an impact on the PDFs, the correlation between the cross
...
quark PDFs in the proton for the central region ($\yboost < 1, \ystar < 1$) and the
...
\begin{figure}[p]
  ...
  \caption[Correlation between dijet cross section and PDFs]{
    ...
    bin $\yboost < 1 and \ystar < 1$ (left) and for the boosted region $2 \leq
    \yboost < 3$ (right).}
...
HERA DIS data is always referred to as HERA-II DIS.} were the basis for the determination of the HERAPDF 2.0 PDF
set.

\xfitter is an open source framework to fit the PDFs to experimental data based
...
terms which were found to better describe the additionally included jet
measurement. The PDFs parameterization is defined at the starting scale $Q_0$
which is set to $Q_0 = \SI{1.9}{\GeV \square}$ and the five independent PDFs
$xu_v(x)$, $xd_v(x)$, $xg(x)$, $x\bar{U}(x)$ and $x\bar{D}(x)$ are parameterized
as follows:
%
\begin{align}
...
the model uncertainties the following variations on the input parameters are considered:
%
\begin{itemize}
\item The assumption is made that the shape of the strange quark PDF follows the
  ...
  fitted but determined as a fraction of the $xD$ PDF.The strangeness fraction
...
To estimate the influence of the initial PDF parameterization on the fit result, a
more flexible form of parametrization is used. Using the general
...
envelope of all changes to the PDF shape is collected in a parametrization
uncertainty. Furthermore, the variation of the starting scale $Q_0^2$ to
\SI{1.6}{\GeV\square} and \SI{2.2}{\GeV\square} is treated as an 
...
\begin{equation}
  ...
  \left(D_j - T_j - \sum_k^K r_k \beta_{jk} \right) + \sum_k^K r_k^2 +\sum_j \ln \frac{\Delta_{i,\mathrm{stat}}^2 D_j T_j + \Delta_{i,\mathrm{uncor}}^2 T_i^2}{\left( \Delta_{i,\mathrm{stat}}^2 + \Delta_{i,\mathrm{uncor}}^2 \right) D_i^2}
  \label{chi2_nuisance}
\end{equation}

where $C_{ij}^{-1}$ is the inverse covariance matrix of the uncorrelated
systematic and statistical uncertainties~\cite{herafitter_paper}. The given
representation allows to study the influence of the nuisance parameters after
the fit.
...
be examined. Tab.~\ref{tab:pdfconstraints:nuisance} shows the 27 sources of
systematic uncertainty and their pulls. Most of the systematic sources shift by
less than one standard deviation $\sigma$. Three sources exhibit shifts larger
than two standard deviations. While this not surprising due to the gaussian
distribtution of the pulls, no unambiguous reason for these pulls could be
identified. However, the size of these sources are rather small compared to the
dominant sources of uncertainty so that no larger influence on the fit is
expected.
...
\overline D +
x\overline s)$, are much less pronounced.
