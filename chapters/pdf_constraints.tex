...
\chapter{PDF Constraints and Determination of the Strong Coupling Constant}
...
base data set since it covers a large range in $x$ and $Q$ of the kinematic
phase space. By
...
dijet cross section prediction and of the PDF with flavor $f$, respectively.
...
To ensure consistency between the HERA DIS and the dijet cross section calculations, the fits are performed
...
most notable difference is the neglection of the negative gluon term, a
supplemental term allowing the gluon PDF to become negative at low $x$. No
...
\end{align*}
%
Actually, not all of those parameters are fitted. The normalization parameters $A_g$,
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/pdf_constraints/hessianmethod.pdf}
...
$X_i^{\mathrm{dn}}$ denote the result using the upwards and downwards variations of
...
To estimate the influence of the parametrization choice on the fit result, a
...
\section{PDFs Constraints of the Triple-Differential Dijet Cross Section}
...
with and without the CMS dijet data are shown next to each other in
Fig.~\ref{fig:pdfconstraints:split:gluonqsea:19} and Fig.~\ref{fig:pdfconstraints:split:dvaluval:19}
...
Figure~\ref{fig:pdfconstraints:direct:10000} in the appendix
...
features compared to the starting scale $Q_0$, but the uncertainty reduction is
even more pronounced. Finally, an overview of the gluon, sea, u valence and d valence quark PDFs is
...
The fit is set up in the same manner as
