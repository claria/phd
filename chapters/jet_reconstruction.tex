...
measure in Minkowski space. All jets were reconstructed using the efficient algorithms
implemented in the FastJet library~cite{Cacciari:2011ma}.
...
\begin{figure}[htb]
    ...
    \caption{The influence of an infrared emission and a quasi
...
plane~\ref{Cacciari:2008gn}. While cone-based jet algorithms yield a $\pi R^2$
...
(CHS)~\cite{Kirschenmann:2014dla}. Naturally the CHS algorithms can only be
...
remain in the event. The jets are now clustered from this set of particles.
Since the jet identification criteria in the jet selection in the analysis
require at least one charged particle in a jet, the CHS method effectively
reduces the influence of pile-up on real jets as well as completely removing a
